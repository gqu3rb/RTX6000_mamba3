...
class TinyStories(IterableDataset):
    ...
    def __init__(
        ...
        self.max_len_internal = max_length 
        
        self.tokens = []

        self.stream = open(text_path, 'r', encoding='utf-8', errors='ignore')
        
        self._start = 0
        self._end   = self.file_size

    def __len__(self) -> int:
        return self.file_size // (4 * self.max_len_internal)
    ...
    def __iter__(self):
        ...
        target_chunk_size = self.max_len_internal + 1
        
        while self.stream.tell() < self._end:
        
            while len(self.tokens) < target_chunk_size:
            ...
            if len(self.tokens) < target_chunk_size:
                break
            
            chunk = self.tokens[:target_chunk_size]
            self.tokens = self.tokens[self.max_len_internal:] 
            
            prev = torch.tensor(chunk[:-1], dtype=torch.long)
            post = torch.tensor(chunk[1:], dtype=torch.long)
...
class TinyStoriesLightning(LightningDataset):
    ...
    def setup(self, stage: str) -> None:
        ...
        # Replaced Python 3.10 match-case with Python 3.8 if-elif-else
        if stage == 'fit':
            self.train_dataset = TinyStories(
                root=self.root,
                tokenizer=self.tokenizer,
